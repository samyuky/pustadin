...
<head>
    ...
    <title>Daftar Permintaan Data - Admin PUSDATIN</title>
...
<body class="bg-light-100 dark:bg-dark-900 text-gray-800 dark:text-gray-200 min-h-screen transition-colors duration-300 flex flex-col">
    ...
    <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Daftar Permintaan Data</h1>
                    ...
                    @forelse ($dataRequests as $request)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('admin.data-requests.show', $request->id) }}" class="text-primary hover:text-indigo-600 dark:text-secondary dark:hover:text-green-400 mr-3">Detail</a>
                                <form action="{{ route('admin.data-requests.destroy', $request->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus permintaan ini?');">
                                    @csrf
                                    @method('DELETE')
