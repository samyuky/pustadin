...
<head>
    ...
    <title>Daftar Keluhan - Admin PUSDATIN</title>
...
<body class="bg-light-100 dark:bg-dark-900 text-gray-800 dark:text-gray-200 min-h-screen transition-colors duration-300 flex flex-col">
    ...
    <!-- Main Content Section - Daftar Keluhan -->
    <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Daftar Keluhan</h1>
        ...
        <div class="bg-white dark:bg-dark-800 rounded-xl shadow-lg p-6 overflow-x-auto">
            @if($complaints->isEmpty())
                <p class="text-center text-gray-600 dark:text-gray-400">Belum ada keluhan.</p>
            @else
                    ...
                    <tbody class="bg-white dark:bg-dark-800 divide-y divide-gray-200 dark:divide-dark-700">
                        @foreach ($complaints as $complaint)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ $complaint->created_at->format('d M Y H:i') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('admin.complaints.show', $complaint->id) }}" class="text-primary hover:text-indigo-600 dark:text-secondary dark:hover:text-green-400 mr-3">Detail</a>
                                    <form action="{{ route('admin.complaints.destroy', $complaint->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus keluhan ini?');">
                                        @csrf
                                        @method('DELETE')
