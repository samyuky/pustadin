...
<head>
    ...
    <title>Detail Keluhan #{{ $complaint->id }} - Admin PUSDATIN</title>
...
<body class="bg-light-100 dark:bg-dark-900 text-gray-800 dark:text-gray-200 min-h-screen transition-colors duration-300 flex flex-col">
    ...
    <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Detail Keluhan</h1>
        ...
        <div class="bg-white dark:bg-dark-800 rounded-xl shadow-lg p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">ID Keluhan:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $complaint->id }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Subjek Keluhan:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $complaint->subject }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Pelapor:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $complaint->complainant_name ?? 'Anonim' }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Email Pelapor:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $complaint->complainant_email ?? 'Tidak Tersedia' }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Diajukan Pada:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $complaint->created_at->format('d M Y H:i') }}</p>
                </div>
                @if($complaint->resolved_at)
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Diselesaikan Pada:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $complaint->resolved_at->format('d M Y H:i') }}</p>
                </div>
                @endif
                <div>
            ...
            <div class="mb-4">
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Pesan Keluhan:</p>
            ...
            <form action="{{ route('admin.complaints.update_status', $complaint->id) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select id="status" name="status" class="w-full px-4 py-2 border border-gray-300 dark:border-dark-600 rounded-md focus:ring-primary focus:border-primary dark:bg-dark-700 dark:text-gray-200" required>
                        ...
                        <option value="approved" {{ $complaint->status == 'approved' ? 'selected' : '' }}>Disetujui</option>
        ...
        <div class="flex justify-between items-center mt-6">
            <a href="{{ route('admin.complaints.index') }}" class="px-6 py-2 border-2 border-gray-300 dark:border-dark-600 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-dark-700 transition-colors">
                Kembali ke Daftar Keluhan
            </a>
            <form action="{{ route('admin.complaints.destroy', $complaint->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus keluhan ini secara permanen?');">
                @csrf
                @method('DELETE')
