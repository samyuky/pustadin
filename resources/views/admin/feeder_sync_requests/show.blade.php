    ...
    <head>
        ...
        <title>Detail Permintaan Sinkronisasi Feeder - Admin PUSDATIN</title>
        ...
        <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white dark:bg-dark-800 rounded-xl shadow-lg p-6 mb-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    @if($feederSyncRequest->requester_phone)
                    <div>
                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Nomor Telepon:</p>
                        <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $feederSyncRequest->requester_phone }}</p>
                    </div>
                    @endif
                    <div>
                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Jenis Permintaan:</p>
                        <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ ucfirst(str_replace('_', ' ', $feederSyncRequest->request_type)) }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full {{ $statusClass }}">
                            {{ ucfirst($feederSyncRequest->status) }}
                        </span>
                    </div>
                </div>
                <div class="mb-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Deskripsi Permintaan:</p>
                ...
                <form action="{{ route('admin.feeder-sync-requests.update_status', $feederSyncRequest->id) }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="w-full px-4 py-2 border border-gray-300 dark:border-dark-600 rounded-md focus:ring-primary focus:border-primary dark:bg-dark-700 dark:text-gray-200" required>
                            ...
                            <option value="approved" {{ $feederSyncRequest->status == 'approved' ? 'selected' : '' }}>Disetujui</option>
                            <option value="rejected" {{ $feederSyncRequest->status == 'rejected' ? 'selected' : '' }}>Ditolak</option>
