...
<head>
    ...
    <title>Daftar Permintaan Fitur SIMAK - Admin PUSDATIN</title>
...
<body class="bg-light-100 dark:bg-dark-900 text-gray-800 dark:text-gray-200 min-h-screen transition-colors duration-300 flex flex-col">
    ...
    <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Daftar Permintaan Fitur SIMAK</h1>
        ...
        <div class="bg-white dark:bg-dark-800 rounded-xl shadow-lg p-6 overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
                ...
                <tbody class="bg-white dark:bg-dark-800 divide-y divide-gray-200 dark:divide-dark-700">
                    @forelse ($simakFeatureRequests as $request)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('admin.simak-feature-requests.show', $request->id) }}" class="text-primary hover:text-indigo-600 dark:text-secondary dark:hover:text-green-400 mr-3">Detail</a>
                                <form action="{{ route('admin.simak-feature-requests.destroy', $request->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus permintaan ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                Tidak ada permintaan fitur SIMAK.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div class="mt-4">
                {{-- Pagination links will go here if you implement pagination in the controller --}}
                {{ $simakFeatureRequests->links() }}
            </div>
        </div>
    </main>
