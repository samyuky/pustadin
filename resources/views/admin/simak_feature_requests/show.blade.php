...
<head>
    ...
    <title>Detail Permintaan Fitur SIMAK - Admin PUSDATIN</title>
...
<body class="bg-light-100 dark:bg-dark-900 text-gray-800 dark:text-gray-200 min-h-screen transition-colors duration-300 flex flex-col">
    ...
    <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Detail Permintaan Fitur SIMAK</h1>
        ...
        <div class="bg-white dark:bg-dark-800 rounded-xl shadow-lg p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $simakFeatureRequest->id }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Nama Pemohon:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $simakFeatureRequest->requester_name }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Email Pemohon:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $simakFeatureRequest->requester_email }}</p>
                </div>
                @if($simakFeatureRequest->requester_phone)
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Nomor Telepon:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $simakFeatureRequest->requester_phone }}</p>
                </div>
                @endif
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Judul Permintaan:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $simakFeatureRequest->feature_name }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Deskripsi Fitur:</p>
                    <p class="text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ $simakFeatureRequest->description }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Prioritas:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ ucfirst($simakFeatureRequest->priority) }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Tanggal Diajukan:</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $simakFeatureRequest->created_at->format('d M Y H:i') }}</p>
                </div>
                <div>
                    ...
                    <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full {{ $statusClass }}">
                        {{ ucfirst($simakFeatureRequest->status) }}
                    </span>
                </div>
            </div>
            @if($simakFeatureRequest->attachment_path)
            <div class="mb-4">
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Lampiran:</p>
                <a href="{{ Storage::url($simakFeatureRequest->attachment_path) }}" target="_blank" class="text-primary hover:underline dark:text-secondary">
        ...
        <div class="bg-white dark:bg-dark-800 rounded-xl shadow-lg p-6 mb-6">
            ...
            <form action="{{ route('admin.simak-feature-requests.update_status', $simakFeatureRequest->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <select id="status" name="status" class="w-full px-4 py-2 border border-gray-300 dark:border-dark-600 rounded-md focus:ring-primary focus:border-primary dark:bg-dark-700 dark:text-gray-200" required>
                        <option value="pending" {{ $simakFeatureRequest->status == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="approved" {{ $simakFeatureRequest->status == 'approved' ? 'selected' : '' }}>Disetujui</option>
                        <option value="rejected" {{ $simakFeatureRequest->status == 'rejected' ? 'selected' : '' }}>Ditolak</option>
                        <option value="completed" {{ $simakFeatureRequest->status == 'completed' ? 'selected' : '' }}>Selesai</option>
                    </select>
                </div>
        ...
        <div class="flex justify-between items-center mt-6">
            <a href="{{ route('admin.simak-feature-requests.index') }}" class="px-6 py-2 border-2 border-gray-300 dark:border-dark-600 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-dark-700 transition-colors">
                Kembali ke Daftar
            </a>
            <form action="{{ route('admin.simak-feature-requests.destroy', $simakFeatureRequest->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus permintaan ini secara permanen?');">
                @csrf
                @method('DELETE')
